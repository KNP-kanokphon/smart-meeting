...
export const PartyQRcodeCheckout: React.FC<Props> = ({
  baseURL,
}): React.ReactElement => {
  ...
  const onFinish = () => {
    if (getUserid === '') {
      ...
      Modal.success({
        title: 'ขอบคุณสำหรับการเข้าร่วม ',
        icon: <ExclamationCircleOutlined />,
        content: 'ยืนยันการรับของชําร่วย',
        async onOk() {
          await DatamanagementService()
            .updaterecivegift(getUserid)
            .then((e: any) => {
              if (e === 0) {
                message.error('ไม่พบรหัสผู้ใช้งานนี้');
              } else {
                message.success('เสร็จสิ้น');
